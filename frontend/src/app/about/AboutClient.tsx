...
export default function AboutClient({ cmsData }: { cmsData: CMSData }) {
  const { t } = useTranslation();
  const heroData = cmsData.hero || {};
  const heroBadge = heroData.badge || t("about.hero.badge");
  const heroTitle1 = heroData.title1 || t("about.hero.title1");
  const heroTitle2 = heroData.title2 || t("about.hero.title2");
  const heroDesc = heroData.description || t("about.hero.desc");
  const heroImage = heroData.image || "/images/Section 2/Section 2_Baby.png";

  const vision = cmsData.vision || {};
            ...
            <div className="space-y-8">
              ...
              <ScrollReveal delay={0.2}>
                <h1 className="font-headline-xl text-headline-xl text-primary leading-tight">
                  {heroTitle1} <span className="text-gradient-primary">{heroTitle2}</span>
                </h1>
              </ScrollReveal>
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
          <ScrollReveal>
            <div className="text-center mb-16 space-y-4">
              <h2 className="font-headline-lg text-headline-lg text-primary">{vision.title || t("about.vision.title")}</h2>
              <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl mx-auto">
                {vision.description || t("about.vision.desc")}
              </p>
            </div>
          </ScrollReveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter">
            ...
            <ScrollReveal className="md:col-span-2">
              <div className="relative group overflow-hidden rounded-3xl bg-primary-container/10 p-12 flex flex-col justify-end min-h-[400px] shadow-lg border border-primary-container/20 hover:shadow-xl transition-all h-full">
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <h3 className="text-4xl font-bold text-primary">{vision.heritageTitle || t("about.vision.heritage.title")}</h3>
                  <p className="text-on-surface-variant max-w-md text-lg">
                    {vision.heritageDesc || t("about.vision.heritage.desc")}
                  </p>
                </div>
            ...
            <ScrollReveal delay={0.2}>
              <div className="glass-panel p-10 rounded-3xl border border-secondary-container/30 flex flex-col items-center text-center space-y-6 h-full hover:-translate-y-2 transition-transform duration-300">
                ...
                <h3 className="text-2xl font-bold text-primary">{vision.purityTitle || t("about.vision.purity.title")}</h3>
                <p className="text-on-surface-variant">
                  {vision.purityDesc || t("about.vision.purity.desc")}
                </p>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.3}>
              <div className="glass-panel p-10 rounded-3xl bg-primary-fixed/10 border border-primary-container/20 flex flex-col items-center text-center space-y-6 h-full hover:-translate-y-2 transition-transform duration-300">
                ...
                <h3 className="text-2xl font-bold text-primary">{vision.natureTitle || t("about.vision.nature.title")}</h3>
                <p className="text-on-surface-variant">
                  {vision.natureDesc || t("about.vision.nature.desc")}
                </p>
              </div>
            </ScrollReveal>
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
          <div className="flex flex-col md:flex-row gap-16 items-center">
            <div className="w-full md:w-1/2">
              <ScrollReveal direction="right">
                <div className="relative rounded-3xl overflow-hidden aspect-square shadow-2xl">
                  <img className="w-full h-full object-contain bg-surface-container-low" src={founder.image || "/images/Section 6/Section 6_Created by Parent.png"} alt="Founder" />
                  <div className="absolute bottom-8 left-8 glass-panel p-8 rounded-2xl max-w-sm shadow-xl">
                    <p className="text-primary font-bold italic text-lg leading-relaxed">
                      "{founder.quote || t("about.founder.quote")}"
                    </p>
                    <p className="mt-4 font-bold text-sm text-on-surface-variant">- {founder.author || t("about.founder.author")}</p>
                  </div>
                </div>
              </ScrollReveal>
            </div>
            <div className="w-full md:w-1/2 space-y-10">
              <ScrollReveal delay={0.1}>
                <h2 className="font-headline-lg text-headline-lg text-primary">{founder.title1 || t("about.founder.title1")} <span className="text-gradient-primary">{founder.title2 || t("about.founder.title2")}</span></h2>
              </ScrollReveal>
              <ScrollReveal delay={0.2}>
                <div className="space-y-6 text-on-surface-variant font-body-md text-lg leading-relaxed whitespace-pre-line">
                  <p>{founder.p1 || t("about.founder.p1")}</p>
                  {founder.p2 && <p>{founder.p2}</p>}
                </div>
              </ScrollReveal>
              <ScrollReveal delay={0.3}>
                <div className="grid grid-cols-2 gap-8 pt-8 border-t border-outline-variant/30">
                  <div>
                    ...
                    <p className="text-sm font-label-md text-on-surface-variant uppercase tracking-wider">{founder.stat1 || t("about.founder.stat1")}</p>
                  </div>
                  <div>
                    <p className="text-4xl font-bold text-primary mb-1">15+</p>
                    <p className="text-sm font-label-md text-on-surface-variant uppercase tracking-wider">{founder.stat2 || t("about.founder.stat2")}</p>
                  </div>
                </div>
              </ScrollReveal>
            </div>
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
          <ScrollReveal>
            <div className="bg-primary-container/20 rounded-3xl p-12 md:p-24 text-center space-y-10 relative overflow-hidden shadow-lg border border-primary-container/30">
              ...
              <h2 className="font-headline-xl text-headline-xl text-primary relative z-10">{cta.title || t("about.cta.title")}</h2>
              <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto relative z-10 text-lg">
                {cta.desc || t("about.cta.desc")}
              </p>
              <div className="flex justify-center gap-4 relative z-10 pt-4">
                <Button variant="primary">{cta.button || t("about.cta.button")}</Button>
              </div>
            </div>
          </ScrollReveal>
        </div>
