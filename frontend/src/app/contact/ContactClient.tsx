...
export default function ContactClient({ headerData, infoData, formDataSettings }: { headerData: any, infoData: any, formDataSettings: any }) {
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-6">
          <ScrollReveal>
            <span className="inline-block px-4 py-1.5 rounded-full bg-secondary-container text-on-secondary-container font-label-md text-sm shadow-sm">
              {headerData?.badge || t("contact.badge")}
            </span>
            <h1 className="text-5xl md:text-6xl font-headline-xl text-primary mt-6 leading-tight">
              {headerData?.title1 || t("contact.title1")} <span className="text-gradient-primary">{headerData?.title2 || t("contact.title2")}</span>
            </h1>
            <p className="text-xl text-on-surface-variant mt-4">
              {headerData?.description || t("contact.desc")}
            </p>
          </ScrollReveal>
        </div>
          ...
          <div className="lg:w-1/3 space-y-8">
            <ScrollReveal direction="left" delay={0.1}>
              <div className="glass-panel p-8 rounded-3xl space-y-6 hover:-translate-y-1 transition-transform border border-outline-variant/20 shadow-lg">
                ...
                <div>
                  <h3 className="font-bold text-xl text-primary mb-2">{infoData?.emailTitle || t("contact.email.title")}</h3>
                  <p className="text-on-surface-variant mb-2">{infoData?.emailDesc || t("contact.email.desc")}</p>
                  <a href={`mailto:${infoData?.emailValue || "hello@lolunacare.com"}`} className="text-primary font-bold text-lg hover:underline">{infoData?.emailValue || "hello@lolunacare.com"}</a>
                </div>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal direction="left" delay={0.2}>
              <div className="glass-panel p-8 rounded-3xl space-y-6 hover:-translate-y-1 transition-transform border border-outline-variant/20 shadow-lg">
                ...
                <div>
                  <h3 className="font-bold text-xl text-primary mb-2">{infoData?.callTitle || t("contact.call.title")}</h3>
                  <p className="text-on-surface-variant mb-2">{infoData?.callDesc || t("contact.call.desc")}</p>
                  <a href={`tel:${infoData?.callValue?.replace(/[^0-9+]/g, '') || "+628112345678"}`} className="text-primary font-bold text-lg hover:underline">{infoData?.callValue || "+62 811 234 5678"}</a>
                </div>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal direction="left" delay={0.3}>
              <div className="glass-panel p-8 rounded-3xl space-y-6 hover:-translate-y-1 transition-transform border border-outline-variant/20 shadow-lg">
                ...
                <div>
                  <h3 className="font-bold text-xl text-primary mb-2">{infoData?.locTitle || t("contact.location.title")}</h3>
                  <p className="text-on-surface-variant leading-relaxed">
                    {infoData?.loc1 || t("contact.location.l1")}<br />
                    {infoData?.loc2 || t("contact.location.l2")}<br />
                    {infoData?.loc3 || t("contact.location.l3")}
                  </p>
                </div>
              </div>
            </ScrollReveal>
          ...
          <div className="lg:w-2/3">
            <ScrollReveal direction="right" delay={0.2}>
              <div className="glass-panel p-10 md:p-14 rounded-3xl shadow-2xl border border-white/60 bg-white/50 relative overflow-hidden">
                ...
                <h2 className="text-3xl font-bold text-primary mb-8 relative z-10">{formDataSettings?.title || t("contact.form.title")}</h2>
                ...
                {isSuccess ? (
                  <div className="bg-secondary-fixed/30 border border-secondary-fixed text-on-secondary-fixed-variant p-8 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 animate-fade-in-up min-h-[400px]">
                    ...
                    <h3 className="text-2xl font-bold">{formDataSettings?.successTitle || t("contact.form.successTitle")}</h3>
                    <p className="text-lg">{formDataSettings?.successDesc || t("contact.form.successDesc")}</p>
